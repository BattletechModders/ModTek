    ...
    public static class Injector
    {
        ...
        public static int Main(string directory)
        {
            try
            {
                ...
                SayHeader();
                var managedDirectory = Path.GetDirectoryName(typeof(BattleTech.GameInstance).Assembly.Location);
                // find managed directory, setup assembly resolver to look there
                Log.M.TWL(0,Path.GetDirectoryName(typeof(BattleTech.GameInstance).Assembly.Location));
                ...
                var modTekDLLPath = Path.Combine(directory, MODTEK_DLL_FILE_NAME);
                var modDirectory = Path.Combine(directory, "..");
                string HarmonySrcDLLPath = Path.Combine(directory, HARMONY_DLL_FILE_NAME);
